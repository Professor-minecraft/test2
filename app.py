...
def get_ai_context(df):
    """Convert dataframe to CSV string context for Gemini, limited to 100 rows."""
...
def get_best_gemini_model():
    """Dynamically get the best available Gemini model for text generation."""
    available_models = []
    try:
        available_models = [
    ...
    for p in preferred:
        if p in available_models or f"models/{p}" in available_models:
            print(f"DEBUG: Selected Gemini Model: {p}")
            return p
            
    # If none of the preferred are found, just pick the first one available
    if available_models:
        print(f"DEBUG: Falling back to first available model: {available_models[0]}")
...
with st.sidebar:
    ...
    ai_mode = st.toggle("Enable AI Mode (Gemini)", value=True)
...
if submitted:
    if not query:
        ...
        ans = None
        use_ai_ans = None
        use_ai = ('ai_mode' in locals() and ai_mode) and bool(os.getenv("GOOGLE_API_KEY"))
        
        if 'ai_mode' in locals() and ai_mode and not os.getenv("GOOGLE_API_KEY"):
            st.warning("⚠️ AI Mode is ON, but no Gemini API Key is provided. Falling back to rule-based system.")
            
        if use_ai:
            with st.spinner("Analyzing with AI..."):
                try:
                    data_context = get_ai_context(filtered_df)
                    
                    # Dynamically get best model
...
'''
                    ...
                    use_ai_ans = response.text
                    q_lower = "__ai_mode_skip__" # Prevent matching rule-based if/elif checks
                except Exception as e:
                    st.error(f"⚠️ API Error: {str(e)}. Falling back.")
                    use_ai = False
        ...
        # If AI was used, override with AI response
        if use_ai:
            ans = use_ai_ans
